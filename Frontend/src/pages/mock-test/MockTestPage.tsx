...
const MockTestPage = () => {
  ...
  useEffect(() => {
    const fetchExamData = async () => {
      ...
      try {
        const response = await fetch(`https://enlightiq.enlightiq.in/api/matchsets/${id}/details`);
        if (!response.ok) {
  ...
  const handleSubmitTest = useCallback(async () => {
    ...
    try {
      // Submit to API
      const response = await fetch('https://enlightiq.enlightiq.in/api/matchsets/submit', {
        method: 'POST',
        headers: {
