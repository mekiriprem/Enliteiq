...
const BlogManagement: React.FC<BlogManagementProps> = ({ userType }) => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response = await fetch('https://enlightiq.enlightiq.in/api/blogs');
        if (!response.ok) {
  ...
  const handleDeleteBlog = async (blogId: string, blogTitle: string) => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`https://enlightiq.enlightiq.in/api/blogs/${blogId}`, {
        method: 'DELETE',
        headers: {
    ...
    if (newBlog.title && newBlog.content && newBlog.author && newBlog.category) {
      try {
        ...
        if (imageUploadType === 'file' && imageFile) {
          ...
          const response = await fetch(`https://enlightiq.enlightiq.in/api/blogs/${editingBlogId}`, {
            method: 'PUT',
            body: formData,
          });
