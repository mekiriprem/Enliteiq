...
const SalesTeam: React.FC = () => {
  ...
  const fetchSalesPersons = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://enlightiq.enlightiq.in/api/salesman/all');
  ...
  const handleAddSalesPerson = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://enlightiq.enlightiq.in/api/salesman/register', {
        method: 'POST',
        headers: {
  ...
  const handleStatusChange = async (id: number, newStatus: 'active' | 'inactive') => {
    try {
      const response = await fetch(`https://enlightiq.enlightiq.in/api/salesman/${id}/status?status=${newStatus}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
