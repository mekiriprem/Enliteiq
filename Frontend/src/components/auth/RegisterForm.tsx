...
const RegisterForm = () => {
  ...
  useEffect(() => {
    const fetchSchools = async () => {
      try {
        const response = await fetch("https://enlightiq.enlightiq.in/api/schools/active");
        if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response = await fetch(" https://enlightiq.enlightiq.in/api/signup", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        if (response.status === 415) {
          ...
          response = await fetch(" https://enlightiq.enlightiq.in/api/signup", {
            method: "POST",
